...
def val(model, criterion, dataloader, device):
    ...
    for data in dataloader:
        head_pairs, tail_pairs, rel, head_feature, tail_feature, label = [d.to(device) for d in data]    

        with torch.no_grad():
            pred, all_loss = model((head_pairs, tail_pairs, rel, head_feature, tail_feature), label)
...
def main():
    ...
    matlab_script = "ggraph.m"

    command = 'matlab -nodisplay -nosplash -nodesktop -r "run(\'{}\');exit;"'.format(matlab_script)
    ...
    model = SA_DDI(node_dim, edge_dim, n_iter=n_iter, batch_size = batch_size*2)#.cuda()   
    model.to(device)
...
'''
python train.py --fold 0 --save_model --batch_size 256 > test.txt
'''